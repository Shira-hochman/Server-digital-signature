...
// הוספתי הגדרת CORS שמאפשרת גישה מדומיינים של Vercel
app.use(cors({
    origin: (origin, callback) => {
      if (!origin || origin.endsWith('.vercel.app') || origin === 'https://client-side-digital-signature.vercel.app' || origin === 'http://localhost:3000') {
        callback(null, true);
      } else {
...
// Endpoint חדש: לטיפול בשגיאת "Cannot GET /"
app.get('/', (req, res) => {
    res.status(200).send('Server is running and healthy. Use the /upload endpoint to upload files.');
});
